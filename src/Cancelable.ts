export class Cancelable<a> {
  constructor(public promise: Promise<a>, private _token: { cancel(): void }) {}
  ...
  cancel() {
    ...
    return new Cancelable(this.promise.catch(f), this._token);
  }

  then<b>(f: (_: a) => b | PromiseLike<b>, g?: (_: any) => b | PromiseLike<b>): Cancelable<b> {
    return new Cancelable(this.promise.then(f, g), this._token);
  }
