abstract class Either$abstract<L, R> {
  ...
  chain<B>(f: (_: R) => Either<L, B>): Either<L, B> {
    return this.either(() => (this as unknown) as Left<L>, f);
  }
  chainLeft<B>(f: (_: L) => Either<B, R>): Either<B, R> {
    return this.either(f, () => (this as unknown) as Right<R>);
  }
...
type EitherConstructor = Either$static;
